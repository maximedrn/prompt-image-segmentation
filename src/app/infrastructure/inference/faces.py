...
:func:`app.domain.certainly_adult`: the age estimator classifies
...
def _winning_label(
    ...
    index: int = int(
        logits.float().argmax(-1)[FaceDetectorDefaults.first_result]
    )
...
class YuNetFaceAnalyser:
    ...
    def _crops(self, image: SourceImage) -> list[NDArray[uint8]]:
        ...
        for row in detections:
            ...
            if min(width, height) < FaceDetectorDefaults.minimum_crop_pixels:
                continue
    ...
    def analyse(self, image: SourceImage) -> PersonPayload:
        ...
        with exclusive_device(FaceDetectorDefaults.operation):
            genders: list[Gender] = []
            bands: list[AgeBand] = []
            for crop in self._crops(image):
                ...
                bands.append(Labels.age_bands.get(age_label, AgeBand.TEEN))
                genders.append(
                    Gender.MALE
                    if gender_label == Labels.male
                    else Gender.FEMALE
                )
...
def load_face_analyser(policy: FacePolicy) -> YuNetFaceAnalyser:
    ...
    weights: str = fetching(
        Checkpoints.face_detector.repository,
        lambda: hf_hub_download(
            Checkpoints.face_detector.repository,
            Checkpoints.face_detector.filename,
            revision=Checkpoints.face_detector.revision,
        ),
    )
    try:
        detector: FaceDetectorYN = FaceDetectorYN.create(
            weights,
            "",
            FaceDetectorDefaults.initial_size,
            score_threshold=policy.score_threshold,
            nms_threshold=FaceDetectorDefaults.nms_threshold,
            top_k=FaceDetectorDefaults.top_k,
        )
    except Exception as error:
        raise ModelUnavailable(
            model=Checkpoints.face_detector.repository, detail=str(error)
        ) from error

    age_processor: ViTImageProcessor = fetching(
        Checkpoints.age_estimator.repository,
        lambda: ViTImageProcessor.from_pretrained(
            Checkpoints.age_estimator.repository,
            revision=Checkpoints.age_estimator.revision,
        ),
    )
    ...
    gender_processor: SiglipImageProcessor = fetching(
        Checkpoints.gender_estimator.repository,
        lambda: SiglipImageProcessor.from_pretrained(
            Checkpoints.gender_estimator.repository,
            revision=Checkpoints.gender_estimator.revision,
        ),
    )
