...
The only module that knows about Pillow and OpenCV. Decoding is a
validation boundary, so hostile bytes are normalised into a typed
failure here rather than raising a library exception upward
(``SKILL.md`` sections 4 and 21).
"""
...
from app.infrastructure.constants import TextEncoding

_DILATION_ITERATIONS: Final[int] = 1
#: A kernel needs at least one pixel, hence the offset on each axis.
_MINIMUM_KERNEL_SIZE: Final[int] = 1
...
class OpenCvMaskDilator:
    ...
    def dilate(self, mask: MaskArray, percentage: float) -> MaskArray:
        ...
        kernel: NDArray[uint8] = ones(
            (
                int(height * ratio) + _MINIMUM_KERNEL_SIZE,
                int(width * ratio) + _MINIMUM_KERNEL_SIZE,
            ),
            dtype=uint8,
        )
        dilated: NDArray[uint8] = cv2_dilate(
            mask, kernel, iterations=_DILATION_ITERATIONS
        ).astype(uint8)
        return dilated
